...
class Model(nn.Module):
    ...
    During forward, it returns all three logits.
    """
    def __init__(self):
        super().__init__()
        # Instantiate submodels on CPU by default; we move them to GPU later.
        self.model_small = get_small_model(device="cpu")   # Frozen HRNet-based small model (outputs 19 channels)
...
def main(args):
    ...
    # For the frozen small model, we do not create an optimizer.
    ...
    for epoch in range(args.epochs):
        ...
        # Set small model to eval (frozen), medium and big to train.
        model_small.eval()
        model_medium.train()
        model_big.train()
        ...
        for images, labels in train_dataloader:
            ...
            # ---- Small model (frozen, inference only) ----
            ...
            with torch.no_grad():
                out_small = model_small(images)  # [B, 19, H, W]
            ...
            # Compute loss for logging (HRNet output remains 19 channels)
        ...
        # For confusion matrices: Note that ground truth for small model is in 8 classes.
        num_classes_small = 8
        ...
        with torch.no_grad():
            for i, (images, labels) in enumerate(valid_dataloader):
                ...
                # Remap predictions to 8-class space using remap_small function
